...
def main():
    ...
    train.to_json(
        config.MULTINLI_TRAIN_PREPROCESSED_PATH, orient="records", lines=True
    )
    ...
    dev_matched["prem_char_ids"] = dev_matched["sentence1"].apply(
        lang.sent2char_ids
    )
    dev_matched["hypo_char_ids"] = dev_matched["sentence2"].apply(
        lang.sent2char_ids
    )
    ...
    dev_matched.to_json(
        config.MULTINLI_DEV_MATCHED_PREPROCESSED_PATH,
        orient="records",
        lines=True,
    )
    ...
    dev_mismatched = pd.read_json(config.MULTINLI_DEV_MISMATCHED_PATH, lines=True)
    dev_mismatched["prem_token_ids"] = dev_mismatched["sentence1"].apply(
        lang.sent2ids
    )
    dev_mismatched["hypo_token_ids"] = dev_mismatched["sentence2"].apply(
        lang.sent2ids
    )
    dev_mismatched["prem_char_ids"] = dev_mismatched["sentence1"].apply(
        lang.sent2char_ids
    )
    dev_mismatched["hypo_char_ids"] = dev_mismatched["sentence2"].apply(
        lang.sent2char_ids
    )
    ...
    dev_mismatched.to_json(
        config.MULTINLI_DEV_MISMATCHED_PREPROCESSED_PATH,
        orient="records",
        lines=True,
    )
