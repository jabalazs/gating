...
from .datasets.similarity import (
    fetch_MEN,
    fetch_WS353,
    fetch_SimLex999,
    fetch_MTurk,
...
from .datasets.categorization import (
    fetch_AP,
    fetch_battig,
    fetch_BLESS,
    fetch_ESSLI_1a,
...
def calculate_purity(y_true, y_pred):
    ...
    return 1.0 / len(y_true) * np.sum(np.max(M, axis=1))
...
def evaluate_categorization(w, X, y, method="all", seed=None):
    ...
    if method == "all" or method == "agglomerative":
        best_purity = calculate_purity(
            ...
            AgglomerativeClustering(
                n_clusters=len(set(y)), affinity="euclidean", linkage="ward"
        ...
        for affinity in ["cosine", "euclidean"]:
            for linkage in ["average", "complete"]:
                purity = calculate_purity(
                    y[ids],
                    AgglomerativeClustering(
                        n_clusters=len(set(y)), affinity=affinity, linkage=linkage
                    ).fit_predict(words[ids]),
                )
                logger.debug(
                    "Purity={:.3f} using affinity={} linkage={}".format(
    ...
    if method == "all" or method == "kmeans":
        purity = calculate_purity(
            y[ids],
            KMeans(
                random_state=seed, n_init=10, n_clusters=len(set(y))
            ).fit_predict(words[ids]),
        )
...
def evaluate_on_semeval_2012_2(w):
    ...
    for c in categories:
        ...
        scores = np.dot(
            prot_left - prot_right, (question_left - question_right).T
        )
...
def evaluate_analogy(
    w, X, y, method="add", k=None, category=None, batch_size=100
):
    """
    Simple method to score embedding using SimpleAnalogySolver
    ...
    assert (
        category is None or len(category) == y.shape[0]
    ), "Passed incorrect category list"
...
def evaluate_on_WordRep(w, max_pairs=1000, solver_kwargs={}):
    ...
    for cat in categories:
        ...
        logger.info(
            "Processing {} with {} pairs, {} questions".format(
...
def evaluate_on_all(w):
    ...
    for name, data in iteritems(analogy_tasks):
        ...
        logger.info(
            "Analogy prediction accuracy on {} {}".format(
    ...
    for name, data in iteritems(categorization_tasks):
        ...
        logger.info(
            "Cluster purity on {} {}".format(name, categorization_results[name])
        )
