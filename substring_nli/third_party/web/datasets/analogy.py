...
def fetch_wordrep(subsample=None, rng=None):
    ...
    """
    path = _fetch_file(
        url="https://www.dropbox.com/sh/5k78h9gllvc44vt/AAALLQq-Bge605OIMlmGBbNJa?dl=1",
        data_dir="analogy",
        uncompress=True,
    ...
    wikipedia_dict = glob.glob(
        os.path.join(path, "Pairs_from_Wikipedia_and_Dictionary/*.txt")
    )
    ...
    if subsample:
        ...
        ids = rng.choice(
            range(len(word_pairs)),
            int(subsample * len(word_pairs)),
            replace=False,
        )
    ...
    return Bunch(
        category_high_level=np.array(category_high_level),
...
def fetch_google_analogy():
    ...
    return Bunch(
        X=np.vstack(questions).astype("object"),
...
def fetch_msr_analogy():
    ...
    assert set([c.upper() for c in category]) == set(
        [
            "VBD_VBZ",
            "VB_VBD",
            "VBZ_VBD",
...
def fetch_semeval_2012_2(which="all", which_scoring="golden"):
    ...
    path = _fetch_file(
        url="https://www.dropbox.com/sh/aarqsfnumx3d8ds/AAB05Mu2HdypP0pudGrNjooaa?dl=1",
        data_dir="analogy",
        uncompress=True,
    ...
    train_files = set(glob.glob(os.path.join(path, "train*.txt"))) - set(
        glob.glob(os.path.join(path, "train*_meta.txt"))
    )
    test_files = set(glob.glob(os.path.join(path, "test*.txt"))) - set(
        glob.glob(os.path.join(path, "test*_meta.txt"))
    )
    ...
    for f in files:
        ...
        for line_pl in platinium[1:]:
            ...
            questions[c].append(
                [standardize_string(w) for w in word_pair.split(":")]
            )
    ...
    return Bunch(
        X_prot=_change_list_to_np(prototypes),
