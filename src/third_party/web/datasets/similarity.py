...
def fetch_MTurk():
    ...
    return Bunch(
        X=data[:, 0:2].astype("object"), y=2 * data[:, 2].astype(np.float)
    )
...
def fetch_MEN(which="all", form="natural"):
    ...
    if form == "natural":
        ...
        data = data.apply(
            lambda x: [y if isinstance(y, float) else y[0:-2] for y in x]
        )
    elif form != "lem":
...
def fetch_SimLex999():
    ...
    return Bunch(
        X=X.astype("object"), y=y, sd=sd, conc=conc, POS=POS, assoc=assoc
    )
...
def fetch_VerbSim3500():
    """

    """

    uncompressed_path = _fetch_file(
        "http://people.ds.cam.ac.uk/dsg40/paper/simverb/simverb-3500-data.zip",
        "similarity",
        uncompress=True,
        verbose=0,
    )

    file_path = os.path.join(uncompressed_path, 'data', 'SimVerb-3500.txt')
...
def fetch_Card660():
    """
    https://pilehvar.github.io/card-660/card-660.zip
    """
    uncompressed_path = _fetch_file(
        "https://pilehvar.github.io/card-660/card-660.zip",
        "similarity",
    ...
    file_path = os.path.join(uncompressed_path, 'dataset.tsv')
