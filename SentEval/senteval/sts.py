...
class STSEval(object):
    ...
    def run(self, params, batcher):
        ...
        for dataset in self.datasets:
            ...
            # pearsonr return a tuple (correlation, p_value)
            # spearmanr returns a SpearmanrResult object that we need to turn
            # into a tuple so it's serializable
            (pearson_corr, pearson_pvalue) = pearsonr(sys_scores, gs_scores)
            (spearman_corr, spearman_pvalue) = spearmanr(sys_scores, gs_scores)
            results[dataset] = {'pearson': (pearson_corr, pearson_pvalue),
