...
def get_classif_name(classifier_config, usepytorch):
    if not usepytorch:
        ...
        optim = (
            "adam"
            if "optim" not in classifier_config
            else classifier_config["optim"]
        )
        bs = (
            64
...
class InnerKFoldClassifier(object):
    ...
    def run(self):
        ...
        innerskf = StratifiedKFold(
            n_splits=self.k, shuffle=True, random_state=1111
        )
        count = 0
        for train_idx, test_idx in skf.split(self.X, self.y):
            ...
            self.testresults.append(round(score.item(), 2))
...
class KFoldClassifier(object):
    ...
    def run(self):
        # cross-validation
        logging.info(
            "Training {0} with {1}-fold cross-validation".format(
                self.modelname, self.k
            )
        )
        regs = (
...
class SplitClassifier(object):
    ...
    def run(self):
        logging.info(
            "Training {0} with standard validation..".format(self.modelname)
        )
        regs = (
            [10 ** t for t in range(-5, -1)]
            if self.usepytorch
